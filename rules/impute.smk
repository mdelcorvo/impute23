...
rule oxford2vcfX:
    ...
    params:
        name= expand("{name}",name=NAME)       
    output:
        vcfx=  expand("imputed/{name}.X.vcf",name=NAME)
    conda:
        "envs/plink.yaml"
    shell:
...
rule bcftools_concat:
    input:
        vcf= expand("imputed/{name}.{chr}.vcf",name=NAME,chr=CHR),
    output:
        list= temp("results/vcf_list.txt"),
        vcf= temp(expand("imputed/{name}.imputed.vcf",name=NAME))
    params:
        name= expand("{name}",name=NAME)    
    conda:
        "envs/bcftools.yaml"    
